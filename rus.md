...
Элементы, для которых устрановлен `width` на самом деле *шире*, если у них есть
...
В то время как мобильный Safari поддерживает использование `rem` в любом css-cвойстве,
...
Для клирфикса, вызываемого отдельным класом можно использовать [микро клирфикс][3].
...
Поэтому не имеет смысла устанавливать и то и другое, так как `float` всеравно 
...
Если вы предполи первый вариант (и я бы рекомендовал именно его, так как тег `<button>`
...
`<button>`) для `:focus`. Вероятно это делается для повыщения доступности,
...
и элемент с фиксированным позиционированнием начинает вести себя как элемент с 
`position: absolute;`.
